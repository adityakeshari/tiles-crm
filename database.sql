...
CREATE TABLE products (
  ...
  today_selling_rate NUMERIC(14, 2) NOT NULL DEFAULT 0 CHECK (today_selling_rate >= 0),
  daily_up_limit_percent NUMERIC(8, 2) NOT NULL DEFAULT 2 CHECK (daily_up_limit_percent >= 0),
  daily_down_limit_percent NUMERIC(8, 2) NOT NULL DEFAULT 1 CHECK (daily_down_limit_percent >= 0),
  ...
  operator_discount_cap NUMERIC(8, 2) NOT NULL DEFAULT 0 CHECK (operator_discount_cap >= 0),
  manager_discount_cap NUMERIC(8, 2) NOT NULL DEFAULT 0 CHECK (manager_discount_cap >= 0),
  owner_discount_cap NUMERIC(8, 2) NOT NULL DEFAULT 0 CHECK (owner_discount_cap >= 0),
  ...
  quotation_validity_days INT NOT NULL DEFAULT 0 CHECK (quotation_validity_days >= 0),
...
CREATE INDEX idx_leads_department_status_created_at ON leads(department, status, created_at DESC);
CREATE INDEX idx_leads_business_unit_created_at ON leads(business_unit, created_at DESC);
CREATE INDEX idx_payments_created_at_desc ON payments(created_at DESC);
CREATE INDEX idx_quotations_created_at_desc ON quotations(created_at DESC);
...
CREATE INDEX idx_expenses_date_desc ON expenses(expense_date DESC, created_at DESC);
CREATE INDEX idx_adhesive_claims_created_at_desc ON adhesive_token_claims(created_at DESC);
CREATE INDEX idx_adhesive_claims_status_created_at ON adhesive_token_claims(status, created_at DESC);
CREATE INDEX idx_adhesive_claims_mason_status_created_at ON adhesive_token_claims(mason_id, status, created_at DESC);
